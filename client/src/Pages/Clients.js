...
function Clients() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/clients", {withCredentials: true}).then(res => {
      if (res.data != null) {
        setClientsData(res.data[0].map(t1 => ({
          ...t1,
  ...
  const ClientsTable = () => {
    ...
    const clientsFiltered = clientsData
      ?.filter((val) => {
        ...
        ].some(r => r.includes(filterId))) {
          return val
        }
      }).reverse();
    const totalClients = clientsFiltered.length;
    const computedClients = clientsFiltered.slice((currentPage - 1) * itemsPerPage, (currentPage - 1) * itemsPerPage + itemsPerPage);
    const computedClientsLength = computedClients.length;

    return (
      <> < div className = "tableResultsWrap" > <div className="resultsSpan">
     Showing
      <font className="resultsBold"> {computedClientsLength} </font>
      of
      <font className="resultsBold"> {totalClients} </font>
      results</div>
    <Pagination total={totalClients} itemsPerPage={itemsPerPage} currentPage={currentPage} onPageChange={page => setCurrentPage(page)}/>
  </div>
  <table>
    <thead>
      <tr>
    ...
    <tbody>
      {
        computedClients.map((client, i) => {
          return (<tr key={i}>
            <td>
            <font className="maincolor">#</font>{client.client_id}</td>
            <td>{client.client}</td>
            <td>{client.phone}</td>
            <td>{client.city}</td>
            <td>{
  ...
  const AddClients = () => {
    ...
    return (<Popup trigger={buttonPopup} setTrigger={setButtonPopup}>
        ...
        <div className="addNewOrderWrap">
          <div className="addNewOrderForm">
            <div className="orderDetails">
              ...
              <div className="input-group">
                <input type="textarea" placeholder="Client details" className="orderDetailsInput" value={clientDetails.clientDetails} onChange={e => setClientDetails({
                    ...clientDetails,
              ...
              <div className="input-group">
                ...
                <input type="text" placeholder="Street, home/appartment number" className="orderDetailsInput orderDetailsInputHalf" value={clientDetails.street} onChange={e => setClientDetails({
                    ...clientDetails,
